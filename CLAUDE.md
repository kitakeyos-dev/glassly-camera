...
- The capture button supports short-tap (countdown) and long-press (burst) via `burstHoldTimer` / `burstIntervalId` in [js/main.js](js/main.js). Keyboard Space mirrors short-tap (goes through `runCountdownCapture`), and is ignored while typing in inputs/textareas/contenteditable to keep the Space character usable in the text editor.
